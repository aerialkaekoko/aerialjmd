...
<body>
    <div class="original-invoice">
      ...
      <main>
         ...
         <table width="100%" class="table2" style="margin-top:28px">
             <!-- <tr>
         ...
         <table width="100%" style="">
             <tr height="60px" class="table3">
                 ...
                 <th width="15%" class="text-center">Medical Expense</th>
                 <th width="15%" class="text-center">Interpreter Fee 
                </th>
                <th width="20%" class="text-center">Amount
                </th>
             </tr>
             
             @foreach($invoice_code as $key =>$desc)
              <tr>
                 <td>{{$key+1}} ) {{ $desc->user->family_name ?? '' }} {{ $desc->user->name ?? '' }}<br/> {{ $desc->medical_info->date_of_visit ?? ''}} </td>
                 <td style="text-align: center;">{{ $desc->qty ?? '' }}</td>
                 <td style="text-align: center;">{{ $desc->medical_info->medical_amount ?? '0'}}
                    @if($invoice->change_currency==1){{ "USD" }}
                    @elseif($invoice->change_currency==2){{ "Baht" }}
                    @elseif($invoice->change_currency==3){{ "MMK" }}
                    @else($invoice->change_currency==2){{ "LAK" }}
                    @endif
                 </td>
                 <td style="text-align: center;">{{ $desc->interpreter_fee ?? '0'}}
                  @if($invoice->change_currency==1){{ "USD" }}
                  @elseif($invoice->change_currency==2){{ "Baht" }}
                  @elseif($invoice->change_currency==3){{ "MMK" }}
                  @else($invoice->change_currency==2){{ "LAK" }}
                 ...
                 <td style="text-align: center;">{{ $desc->medical_info->medical_amount + $desc->interpreter_fee ?? ''}}
                  @if($invoice->change_currency==1){{ "USD" }}
                  @elseif($invoice->change_currency==2){{ "Baht" }}
                  @elseif($invoice->change_currency==3){{ "MMK" }}
                  @else($invoice->change_currency==2){{ "LAK" }}
           ...
           <table width="100%" style="">
             <tr class="table3">
                 <th width="65%" class="text-center" rowspan="5">Exchange Rate: <br/> USD/THB = {{$usd_thb ?? ''}} <br/>
                  <?php 
           ...
           <table width="100%" style="">
             <tr class="table3">
              ...
              <tr>
                 <td style="border-bottom: 1px solid #fff">Please make cheques payable to "BlueAssistance Co., Ltd." or transfer the full amount to the following bank account.
                  </td>
              </tr>
              <tr>
         ...
         <table width="100%" style="">
             ...
             @foreach($invoice_code as $key =>$desc)
              <tr>
                 <td>{{$key+1}} ) {{ $desc->user->family_name ?? '' }} {{ $desc->user->name ?? '' }}<br/> {{ $desc->medical_info->date_of_visit ?? ''}} </td>
                 <td style="text-align: center;">{{ $desc->qty ?? '' }}</td>
                 <td style="text-align: center;">{{ $desc->medical_info->medical_amount ?? '0'}}</td>
                 <td style="text-align: center;">{{ $desc->interpreter_fee ?? '0'}}</td>
                 <td style="text-align: center;">{{ $desc->medical_info->medical_amount + $desc->interpreter_fee ?? ''}} </td>
